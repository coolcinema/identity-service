...
import { Services } from "@coolcinema/api";

Services.identityservice.grpc.IDENTITY_SERVICE.getPrice(request, callback);
...
const server = createServer(async (req, res) => {
  ...
  runInContext(fakeContext, async () => {
    ...
    try {
      ...
      const response = await Services.identity.grpc.IDENTITY_SERVICE.getPrice({
        showtime_id: "123",
      });
